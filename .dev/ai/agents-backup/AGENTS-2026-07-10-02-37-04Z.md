...
| `project manager`, `project planning`, `proposal coverage`, `workstream review`, `execution readiness` | `agent-project-manager/SKILL.md` | Single-project planning governance role: keeps plan→proposal→WO coverage, workstream health, cleanup, and execution-readiness checks while preserving role boundaries (no execution, no blocker queue ownership). |
...
**Trigger phrases:** "gas calendar", "check calendar conflicts", "calendar conflict check", "schedule conflict", "availability check", "add calendar event", "expand recurrence"
...
**Purpose:** Files-first, standards-backed project calendar (RFC 5545 `.ics`): stores events, expands recurrence deterministically, and checks cross-calendar conflicts on request. PULL/on-request only — it never proactively surfaces or notifies about meetings.

**Commands:** `add` (validate → conflict-check → write), `check-conflicts`, `expand`, `availability`, `merge`, `list`, `show`, `cancel`, `validate`. All support `--json`; `--root`/`--now` are global. Exit codes: `0` ok · `2` usage/validation · `3` hard conflict. Full catalog + fully-qualified invocations: the README.
