...
`[verified: <check you ran>]` · `[inferred]` · `[relayed: <who, when>]`.
Untagged means verified or trivially checkable, so the only rule to remember is
**if you cannot honestly leave it untagged, tag it.** A claim never improves by
being copied: the tag travels with it, an untagged claim inherited from another
session IS `[relayed:]`, and only naming a check you ran upgrades a tag.
